...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstFormulaPart = x1 * x2 + y1 * y2;
  const secondFormulaPart =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(firstFormulaPart / secondFormulaPart);
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const lastNums = 10 ** pow;
  const theTail = num % lastNums;
  if (theTail >= 5 * 10 ** (pow - 1)) {
    return num - theTail + lastNums;
  }
  return num - theTail;
}
...
function getSumOfDigits(num) {
  ...
  const result = numArr.reduce((previous, current) => +previous + +current);
  return result;
}
